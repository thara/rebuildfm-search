...
update msg model =
    case msg of
        InputCastName s ->
            ( { model | queryCastName = s }, Cmd.none )

        InputTitle s ->
            ( { model | queryTitle = s }, Cmd.none )

        InputNote s ->
            ( { model | queryNote = s }, Cmd.none )
...
searchEpisodes model =
    let
        url =
            model.apiBaseUrl ++ "/episodes?cast_name=" ++ model.queryCastName ++ "&title=" ++ model.queryTitle ++ "&note=" ++ model.queryNote
